...
def model(input_tensor_batch, class_num):
    ...
    with tf.variable_scope('resnet32') as scope:

        conv0 = tf.nn.relu(tf.layers.batch_normalization(conv2d(input_tensor_batch, 16, 'conv0', kernel_size=3, strides=1),training=False))
        ...
        output = fullyConnect(pool1, class_num, name='output')
